...
public class ErrorLogRepositoryImpl extends RepositoryImpl<ErrorLog> implements ErrorLogRepository {
	...
	@Inject
	public ErrorLogRepositoryImpl(EntityManager entityManager, NginxAgentRunner nginxAgentRunner,
		...
		for (Nginx nginx : nginxRepository.listAllConfigured()) {
			...
			if (nginxResponse.success()) {
				...
				for (FileObject fileObject : nginxErrorLogCollectResponse.getFiles()) {
					try {
						for (String line : IOUtils.readLines(new StringReader(fileObject.getContent()))) {
							Matcher matcher = Pattern
									.compile(
											"([0-9]{4}\\/[0-9]{2}\\/[0-9]{2}\\s[0-9]{2}:[0-9]{2}:[0-9]{2})\\s\\[(.*?)\\]\\s([0-9]{1,})#([0-9]{1,}):(\\s\\*([0-9]{1,}))?\\s(.*)")
									.matcher(line);
							if (matcher.find()) {
								try {
