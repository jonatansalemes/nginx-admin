...
@RunnerType(OperationalSystemType.MAC)
public class MacRunner implements Runner {
	...
	public RuntimeResult start() {
		RuntimeUtils.command(executable() + " -c " + nginx.conf().getAbsolutePath(), nginx.binFolder(), 3);
		return status();
	}

	@Override
	public RuntimeResult stop() {
		RuntimeUtils.command(executable() + " -c " + nginx.conf().getAbsolutePath() + " -s quit", nginx.binFolder(), 3);
		return status();
	}
	...
	public RuntimeResult status() {
		RuntimeResult runtimeResult = RuntimeUtils.command("pgrep " + executable());
	...
	public RuntimeResult testConfig() {
		RuntimeResult runtimeResult = RuntimeUtils
				.command(executable() + " -c " + nginx.conf().getAbsolutePath() + " -t", nginx.binFolder());
		if (runtimeResult.getRuntimeResultType().equals(RuntimeResultType.SUCCESS)) {
	...
	public RuntimeResult version() {
		return RuntimeUtils.command(executable() + " -c " + nginx.conf().getAbsolutePath() + " -v", nginx.binFolder());
	}

	@Override
	public RuntimeResult reload() {
		return RuntimeUtils.command(executable() + "  -c " + nginx.conf().getAbsolutePath() + " -s reload",
				nginx.binFolder());
	}
