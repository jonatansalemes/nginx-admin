...
public class MacRunner implements Runner {
	...
	public RuntimeResult testConfig() {
		RuntimeResult runtimeResult = RuntimeUtils.command(executable() + " -c " + nginx.conf().getAbsolutePath() + " -t",
				nginx.binFolder());
		if(runtimeResult.getRuntimeResultType().equals(RuntimeResultType.SUCCESS)){
	...
	public RuntimeResult version() {
		return RuntimeUtils.command(executable() + " -c " + nginx.conf().getAbsolutePath() + " -v",
				nginx.binFolder());
	}

	@Override
	public RuntimeResult reload() {
		return RuntimeUtils.command(executable() + "  -c " + nginx.conf().getAbsolutePath() + " -s reload", nginx.binFolder());
	}
