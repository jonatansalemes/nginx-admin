...
cp /opt/nginx-admin-agent-2.0.2/scripts/red-hat/nginx-admin-agent.sh /etc/init.d/nginx-admin-agent
...
#logs from server
tail -f /opt/nginx-admin-agent-2.0.2/log/console.log
...
#Please access /opt/nginx-admin-agent-2.0.2/conf/nginx-admin-agent.conf and change variable NGINX_AGENT_AUTHORIZATION_KEY=changeit value for one that only you knows and unique for every node that you have installed. You can also change others configurations if you like in this file and restart service to apply new settings
