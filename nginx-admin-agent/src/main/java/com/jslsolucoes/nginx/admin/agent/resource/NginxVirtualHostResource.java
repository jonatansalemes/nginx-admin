...
public class NginxVirtualHostResource {
	...
	public void create(NginxVirtualHostCreateRequest nginxVirtualHostCreateRequest, @Suspended AsyncResponse asyncResponse,
		...
		NginxOperationResult nginxOperationResult = nginxVirtualHostResourceImpl.create(nginxVirtualHostCreateRequest.getUuid(), nginxVirtualHostCreateRequest.getHttps(), nginxVirtualHostCreateRequest.getCertificateUuid(), nginxVirtualHostCreateRequest.getCertificatePrivateKeyUuid(), nginxVirtualHostCreateRequest.getAliases(), nginxVirtualHostCreateRequest.getLocations());
	     ...
	     asyncResponse.resume(Response
				...
				.entity(new NginxVirtualHostCreateResponse(nginxOperationResult.getOutput(), nginxOperationResult.isSuccess()))
				.build());
	}
	
	@PUT
	@Path("{uuid}")
	public void update(@PathParam("uuid") String uuid,NginxVirtualHostUpdateRequest nginxVirtualHostUpdateRequest, @Suspended AsyncResponse asyncResponse) {
		NginxOperationResult nginxOperationResult = nginxVirtualHostResourceImpl.update(uuid, nginxVirtualHostUpdateRequest.getHttps(), nginxVirtualHostUpdateRequest.getCertificateUuid(), nginxVirtualHostUpdateRequest.getCertificatePrivateKeyUuid(), nginxVirtualHostUpdateRequest.getAliases(), nginxVirtualHostUpdateRequest.getLocations());
		asyncResponse.resume(Response
				.ok(new NginxVirtualHostUpdateResponse(nginxOperationResult.getOutput(), nginxOperationResult.isSuccess()))
				.build());
	}
	...
	public void read(@PathParam("uuid") String uuid, @Suspended AsyncResponse asyncResponse) {
		...
		asyncResponse.resume(Response
				.ok(new NginxVirtualHostReadResponse(fileObject))
				.build());
	}
}
