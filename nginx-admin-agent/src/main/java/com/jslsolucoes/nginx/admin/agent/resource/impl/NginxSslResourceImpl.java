...
public class NginxSslResourceImpl {
	...
	public NginxOperationResult delete(String uuid) {
		try {
			FileSystemBuilder.newBuilder().delete().withDestination(ssl(uuid)).execute().end();
	...
	private NginxOperationResult createOrUpdate(String uuid, FileObject fileObject) {
		try {
			FileSystemBuilder.newBuilder().write().withDestination(ssl(uuid)).withCharset("UTF-8")
					.withContent(fileObject.getContent()).execute().end();
	...
	public FileObject read(String uuid) {
		try {
			File ssl = ssl(uuid);
			String content = FileSystemBuilder.newBuilder().read().withDestination(ssl).withCharset("UTF-8").execute();
			return FileObjectBuilder.newBuilder().from(ssl).withCharset("UTF-8").withContent(content).build();
		} catch (Exception e) {
			return null;
		}
	}
